...
5. **校对、审稿、评估译文时**，义注的黑体被解释词必须与本文同处逐字同译，复注的
   与义注同处逐字同译。这是可机械核查的硬约束，不等就是错（见 conventions.md）。
   同时必须**通查全文的语言风格是否一致**——前后不能一段现代汉语、一段半文半白。
   风格默认是现代汉语（见 conventions.md）。
6. **判断依据是巴利原文，译文只作佐证。** 机器生成的译文必须显式标注。
7. **不要把整章往上下文里灌。** 先看体量再决定取多少。
